...
- [ ] **Step 1: Remove `import logging` from `main.py`**
...
- [ ] **Step 2: Remove `config = {}` from `services/ai_service.py`**

  Open `services/ai_service.py`. Around line 35, delete `config = {}`. This dict is declared inside `generate_text()` but never read.

- [ ] **Step 3: Remove `repo_to_espn` dict from `services/live_score_service.py`**
...
- [ ] **Step 4: Remove duplicate `import time` from `services/draft_service.py`**
...
- [ ] **Step 1: Add Annotated + Path imports**
...
- [ ] **Step 2: Change `/api/progress/{season}/{week}` signature**
...
- [ ] **Step 5: Check JS for "latest" usage**
...
- [ ] **Step 1: Add timeout to subprocess.run**
...
- [ ] **Step 2: Replace call sites in `routes/api_routes.py`**
...
- [ ] **Step 3: Replace call sites in `routes/standings_routes.py`, `routes/history_routes.py`, `routes/draft_routes.py`**
...
- [ ] **Step 1: Find all traceback + str(e) patterns**
...
- [ ] **Step 2: Ensure each file has a logger**
...
- [ ] **Step 3: Replace the pattern in every match**
...
- [ ] **Step 2: Add CORS_ORIGINS to local .env**
...
- [ ] **Step 6: Deploy Batch 3 (run `/deploy`)**
...
- [ ] **Step 1: Create the test file with baseline tests**
...
- [ ] **Step 1: Replace the `.apply()` call with `np.where`**
...
- [ ] **Step 1: Replace the loop implementation**
...
- [ ] **Step 1: Write a correctness test for `get_enriched_schedule`**
...
- [ ] **Step 2: Run the test to confirm it passes with current implementation**
...
- [ ] **Step 3: Add `load_data_season(year)` to `services/data_service.py`**
...
- [ ] **Step 4: Run correctness test again**
...
- [ ] All new tests were written before implementation (watched fail first)
- [ ] `pytest tests/ -q` passes with zero failures
- [ ] No `traceback.print_exc()` or bare `str(e)` returned to users remains in modified files
- [ ] No `import traceback` left unused in modified files
- [ ] MFA `mfa_token` stored value is 64-char hex, not a 6-digit string
- [ ] CORS middleware present in `main.py`, no wildcard `*`
- [ ] `CORS_ORIGINS` set in `.env` and documented for Cloud Run
- [ ] All 7 GitHub issues closed with explanatory comments
