...
**Write path:** `cache_builder.py`'s daily `build_year()` now writes these projections to Firestore's `preseason_predictions` collection via `set_preseason_predictions()` (`services/db_service.py`) — previously this collection was only ever written by a human manually running `predict_season.py`. The write is gated to `year >= current_year` (or `--force`): a completed past season is skipped entirely, never regenerated, because `set_preseason_predictions()`'s per-team lock check would otherwise treat existing historical docs (written by `predict_season.py`, which never sets a `locked` field) as unlocked and silently overwrite them. Once a season's `final_flag` is true, the docs it writes are stamped `locked=True` and left alone by subsequent runs unless `--force` is passed.
